...
static const char* KITN[3]={"ACOU","808","909"};
...
static bool playing=false, helpOn=false;
...
static bool loadWavBuf(const char* path, int16_t** outBuf, uint32_t* outLen, uint32_t* outRate){
  ...
  uint32_t frames=dSz/(2*ch); if(frames>SAMP_MAX)frames=SAMP_MAX; if(frames==0){f.close();return false;}
...
static void saveSong(){
  ...
  for(int i=0;i<STEPS;i++)s.phTrig[i]=phTrig[i]?1:0; for(int t=0;t<2;t++)for(int i=0;i<5;i++)s.fx[t][i]=fx[t][i]?1:0; memcpy(s.phStepFx,phStepFx,sizeof(phStepFx)); memcpy(s.phStepLen,phStepLen,sizeof(phStepLen)); memcpy(s.phStepOff,phStepOff,sizeof(phStepOff)); memcpy(s.phStepPit,phStepPit,sizeof(phStepPit));
  ...
  File f=SD.open(p,FILE_WRITE); if(f){ f.write((uint8_t*)&s,sizeof(s)); f.close(); snprintf(msg,24,"SAVED S%d",slot+1);} else snprintf(msg,24,"SAVE FAIL");
  msgUntil=millis()+900;
}
static void loadSong(){
  char p[32]; snprintf(p,32,"/songs/SONG%d.dat",slot+1); File f=SD.open(p,FILE_READ);
  if(f && f.size()==sizeof(Song)){ Song s; f.read((uint8_t*)&s,sizeof(s)); f.close();
    if(s.magic==0x53474E39){ memcpy(pat,s.pat,sizeof(pat)); memcpy(drum,s.drum,sizeof(drum)); memcpy(prob,s.prob,sizeof(prob)); memcpy(trackVol,s.trackVol,sizeof(trackVol));
      for(int i=0;i<STEPS;i++)phTrig[i]=s.phTrig[i]; for(int t=0;t<2;t++)for(int i=0;i<5;i++)fx[t][i]=s.fx[t][i]; memcpy(phStepFx,s.phStepFx,sizeof(phStepFx)); memcpy(phStepLen,s.phStepLen,sizeof(phStepLen)); memcpy(phStepOff,s.phStepOff,sizeof(phStepOff)); memcpy(phStepPit,s.phStepPit,sizeof(phStepPit));
...
static void draw(){
  ...
  else if(activeTrack==2){ spr.setTextColor(INK); spr.setCursor(98,3); spr.printf("KIT %s",KITN[kit]); }
  ...
  { int at=(activeTrack<2)?activeTrack:0, ot=1-at;
    for(int c=0;c<PAGEW;c++){ int s=base+c,x=gx+c*colw; bool cur=(s==curStep&&playing);
      ...
      for(int k=0;k<MAXN;k++){ int n=pat[at][s][k]; if(n<0)continue; int y=pitchY(n,gy,gh); spr.fillCircle(x+colw/2,y+2,3,cur?INK:pcolOf(n)); }
...
static void drawHelp(){
  ...
  spr.fillScreen(BG);
  spr.setTextColor(ACC); spr.setTextSize(2); spr.setCursor(6,4); spr.print("KEYS"); spr.setTextSize(1);
  spr.setTextColor(GREY); spr.setCursor(56,8); spr.print("ctrl=close   ` =menu");
  const char* L[]={
    "enter=play/stop space=stop tab=track",
    ", . =cursor  notekey=write  del=clear",
    "notes zxcvbnm sdgh#  qwertyuiop",
    "A/B 1-4wave 0sample ;'vol fn+;'tone",
    "FX(A/B) 5DLY 6OCT 7SUB 8-5TH 9CHO",
    "DRUM z..q=8drums  0=kit  ;'=vol",
    "PHRASE 0pick z=trig 5/6start 7/8len",
    "chord fn+zxcvbn=C Dm Em F G Am",
    "fn+,.=prob fn+k=swing fn+[ ]=looplen",
    "f=crush k=tape a=arp l=rev fn+l=seqrev",
    "song -/=slot  /save  \\load" };
...
static void trigStep(int s){
  ...
  if(arpRate==0) for(int t=0;t<2;t++){ if(prob[t][s]<100 && (int)random(0,100)>=prob[t][s]) continue;   // 琶音开时旋律交给arp
    for(int k=0;k<MAXN;k++){ int n=pat[t][s][k]; if(n>=0) noteWithFx(t,n,g,stepMs); } }
  seSetDrumVol(trackVol[2]);
  for(int r=0;r<8;r++) if(drum[s]&(1<<r)) seDrum(r,kit);
...
void seqEnter(){
  ...
  trackVol[0]=trackVol[1]=0.85f; trackVol[2]=1.0f; trackVol[3]=1.0f; toneV=10; crushV=0; seSetCrush(0); tapeActive=false; seSetBend(1.0f); arpRate=0; arpIdx=0; revPlay=false; for(int i=0;i<STEPS;i++){phStepFx[i]=0;phStepLen[i]=0;phStepOff[i]=0;phStepPit[i]=0;}
  if(trackBuf[0]){free(trackBuf[0]);trackBuf[0]=NULL;} if(trackBuf[1]){free(trackBuf[1]);trackBuf[1]=NULL;}
...
void seqLoop(){
  ...
  if(kb.wasPressed("ctrl")){ helpOn=!helpOn; if(helpOn)drawHelp(); else draw(); return; }
  if(helpOn){ drawHelp(); return; }
  ...
  if(kb.wasPressed("[")){ if(fn){ if(loopLen>1)loopLen--; if(editStep>=loopLen)editStep=loopLen-1; snprintf(msg,24,"LEN %d",loopLen); msgUntil=millis()+600; } else { bpm-=4; if(bpm<60)bpm=60; } redraw=true; }
  if(kb.wasPressed("]")){ if(fn){ if(loopLen<STEPS)loopLen++; snprintf(msg,24,"LEN %d",loopLen); msgUntil=millis()+600; } else { bpm+=4; if(bpm>200)bpm=200; } redraw=true; }
  if(kb.wasPressed("del")){ if(fn){ for(int s=0;s<STEPS;s++){stepClear(0,s);stepClear(1,s);drum[s]=0;prob[0][s]=prob[1][s]=100;} snprintf(msg,24,"CLEAR ALL"); msgUntil=millis()+700; } else { if(activeTrack<2)stepClear(activeTrack,editStep); else drum[editStep]=0; } redraw=true; }
  for(int i=0;i<4;i++){ char d[2]={(char)('1'+i),0}; if(kb.wasPressed(d)&&activeTrack<2){ trackSrc[activeTrack]=i; redraw=true; } }  // 波形
  if(kb.wasPressed("0")&&activeTrack<2){            // 切录音当音色
    if(listN==0){ snprintf(msg,24,"NO REC/WAV"); }
    else { int cur=trackSrc[activeTrack]; int ni=(cur<4)?0:((cur-4+1)%listN); setTrackSample(activeTrack,ni); char nm[12]; srcName(activeTrack,nm); snprintf(msg,24,"%c:%s",'A'+activeTrack,nm); }
    msgUntil=millis()+800; redraw=true; }
  if(kb.wasPressed("0")&&activeTrack==3){           // 乐句轨选录音
    if(listN==0){ snprintf(msg,24,"NO REC/WAV"); }
    else { int ni=(phraseSrc<0)?0:((phraseSrc+1)%listN); phPick(ni); snprintf(msg,24,"PHR:%s",listName[phraseSrc]); }
    msgUntil=millis()+800; redraw=true; }
  ...
  if(kb.wasPressed("0")&&activeTrack==2){ kit=(kit+1)%3; redraw=true; }
  if(kb.wasPressed("-")){ slot=(slot+NSLOT-1)%NSLOT; redraw=true; }
  if(kb.wasPressed("=")){ slot=(slot+1)%NSLOT; redraw=true; }
  ...
  for(auto k:NK){ if(kb.wasPressed(k)){
    if(fn && activeTrack<2){
      ...
      if(bit>=0){ seSetDrumVol(trackVol[2]); seDrum(bit,kit); drum[editStep]^=(1<<bit); redraw=true; } }   // 按下必响
